...
# 2. Fungsi untuk Memuat Model dari File .pkl
@st.cache_resource
def load_catalog():
    return joblib.load("best_models_catalog.pkl")

# Membaca catalog model
try:
    catalog = load_catalog()
except FileNotFoundError:
    st.error("⚠️ File 'best_models_catalog.pkl' tidak ditemukan di direktori saat ini!")
    st.stop()
except Exception as e:
    st.error(f"⚠️ Gagal membaca file model: {e}")
    st.stop()

# 3. Sidebar Pilihan Sektor
